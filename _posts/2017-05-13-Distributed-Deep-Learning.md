...
Consider the case of a cluster with n workers, where each worker processes m examples, for a total of nm examples processed between averagings. If we process all nm examples on a single machine with learning rate α, our weight update rule is given by:
